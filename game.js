...
function update() {
    ...
    if (frame % PIPE_INTERVAL === 0) {
        let h = 50 + Math.random() * (canvas.height - PIPE_GAP - 150);
    ...
    pipes.forEach(p => {
        ...
        if (bird.x + 10 > p.x && bird.x - 10 < p.x + 50) {
            if (bird.y - 10 < p.y || bird.y + 10 > p.y + PIPE_GAP) endGame();
        }
...
window.onmousedown = (e) => { 
    if(!e.target.classList.contains('exit-btn') && e.target.id !== 'startBtn') { 
        gameReady = true; bird.vy = FLAP; 
    } 
};
